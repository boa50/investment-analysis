...
export default function KpiCard({
    ...
    const delimiterColour = bgTheme === 'light' ? 'bg-gray-600' : 'bg-gray-100'
...
function Text({ size, bgTheme, value, type }: TextProps) {
    ...
    if (type === 'title') {
        textColour = bgTheme === 'light' ? 'text-gray-600' : 'text-gray-300'
        textSize = size === 'small' ? 'text-sm' : 'text-base'
        fontWeight = 'font-normal'
    } else {
        textColour = bgTheme === 'light' ? 'text-gray-800' : 'text-gray-100'
